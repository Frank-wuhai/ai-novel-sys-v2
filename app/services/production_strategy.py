...
@dataclass(frozen=True)
class ProductionStrategyRule:
    name: str
    evaluate: Callable[[ProductionStrategyContext], ProductionStrategyAssessment]
...
def assess_production_strategy(
    ...
    *,
    ...
    context = ProductionStrategyContext(
        session=session,
    ...
    for rule in get_production_strategy_rules():
        assessment = rule.evaluate(context)
        if assessment.action or assessment.intent:
            return assessment
    return ProductionStrategyAssessment(evidence=evidence)


def get_production_strategy_rules() -> list[ProductionStrategyRule]:
    return [
        ...
        ProductionStrategyRule("blocked_chapter_rebuild", _rule_blocked_chapter_rebuild),
        ProductionStrategyRule("comparison_restore_loop", _rule_comparison_restore_loop),
        ProductionStrategyRule("budget_recovery_pingpong", _rule_budget_recovery_pingpong),
        ProductionStrategyRule("near_gate_plateau", _rule_near_gate_plateau),
        ProductionStrategyRule("linear_revision_exhaustion", _rule_linear_revision_exhaustion),
        ProductionStrategyRule("contract_conflict", _rule_contract_conflict),
        ProductionStrategyRule("quality_rebuild_signal", _rule_quality_rebuild_signal),
        ProductionStrategyRule("pass_prediction_rebuild", _rule_pass_prediction_rebuild),
        ProductionStrategyRule("protected_context", _rule_protected_context),
    ]
...
def _rule_active_budget_recovery(ctx: ProductionStrategyContext) -> ProductionStrategyAssessment:
    if not _active_budget_recovery_state(latest_version=ctx.latest_version, brief_text=ctx.brief_text):
...
def _rule_active_trend_recovery(ctx: ProductionStrategyContext) -> ProductionStrategyAssessment:
    if not _active_trend_recovery_state(latest_version=ctx.latest_version, brief_text=ctx.brief_text):
        return _no_strategy(ctx)
    return ProductionStrategyAssessment(
        action="",
...
def _rule_pending_trend_recovery_contract(ctx: ProductionStrategyContext) -> ProductionStrategyAssessment:
    if not _pending_trend_recovery_contract(ctx.brief_text):
        return _no_strategy(ctx)
    return ProductionStrategyAssessment(
        action="",
...
def _rule_narrow_repairable_gate(ctx: ProductionStrategyContext) -> ProductionStrategyAssessment:
    if not ctx.latest_quality or not _latest_failure_is_narrow_and_repairable(ctx.latest_quality):
        return _no_strategy(ctx)
    if _should_defer_for_later(ctx.session, chapter_id=ctx.chapter_id, latest_quality=ctx.latest_quality):
        return _no_strategy(ctx)
    return ProductionStrategyAssessment(
        action="",
...
def _rule_regressed_rebuild_candidate(ctx: ProductionStrategyContext) -> ProductionStrategyAssessment:
    if not ctx.latest_quality or not _selected_rebuild_candidate_regressed(
        ctx.session,
        chapter_id=ctx.chapter_id,
        latest_version=ctx.latest_version,
        latest_quality=ctx.latest_quality,
    ):
        return _no_strategy(ctx)
    return ProductionStrategyAssessment(
...
def _rule_active_rebuild_candidate(ctx: ProductionStrategyContext) -> ProductionStrategyAssessment:
    if not _active_rebuild_candidate_state(latest_version=ctx.latest_version, brief_text=ctx.brief_text):
        return _no_strategy(ctx)
    if _should_defer_for_later(ctx.session, chapter_id=ctx.chapter_id, latest_quality=ctx.latest_quality):
        return _no_strategy(ctx)
    return ProductionStrategyAssessment(
...
def _rule_blocked_chapter_rebuild(ctx: ProductionStrategyContext) -> ProductionStrategyAssessment:
    if not ctx.revision_brief or not _should_defer_for_later(ctx.session, chapter_id=ctx.chapter_id, latest_quality=ctx.latest_quality):
        return _no_strategy(ctx)
    return ProductionStrategyAssessment(
        action="generate_rebuild_candidates",
...
def _rule_comparison_restore_loop(ctx: ProductionStrategyContext) -> ProductionStrategyAssessment:
    if not ctx.revision_brief or not _comparison_restore_loop(ctx.session, chapter_id=ctx.chapter_id):
        return _no_strategy(ctx)
    return ProductionStrategyAssessment(
        action="generate_rebuild_candidates",
...
def _rule_budget_recovery_pingpong(ctx: ProductionStrategyContext) -> ProductionStrategyAssessment:
    if not ctx.revision_brief or not _budget_recovery_pingpong(rows=ctx.rows, brief_text=ctx.brief_text):
        return _no_strategy(ctx)
    return ProductionStrategyAssessment(
        action="generate_rebuild_candidates",
...
def _rule_near_gate_plateau(ctx: ProductionStrategyContext) -> ProductionStrategyAssessment:
    if not ctx.revision_brief or not _score_plateau_near_gate(ctx.scores):
        return _no_strategy(ctx)
    return ProductionStrategyAssessment(
        action="generate_rebuild_candidates",
...
def _rule_linear_revision_exhaustion(ctx: ProductionStrategyContext) -> ProductionStrategyAssessment:
    if not ctx.revision_brief or not _linear_revision_exhausted(rows=ctx.rows, scores=ctx.scores):
        return _no_strategy(ctx)
    return ProductionStrategyAssessment(
        action="generate_rebuild_candidates",
...
def _rule_contract_conflict(ctx: ProductionStrategyContext) -> ProductionStrategyAssessment:
    if not ctx.revision_brief or not _contract_is_conflicted(ctx.brief_text):
        return _no_strategy(ctx)
    return ProductionStrategyAssessment(
...
def _rule_quality_rebuild_signal(ctx: ProductionStrategyContext) -> ProductionStrategyAssessment:
    if not (
        ctx.latest_quality
        and not ctx.latest_quality.passed
        and ctx.revision_brief
        and _latest_report_requires_rebuild(ctx.latest_quality)
        and _can_strategy_escalate_reading_rebuild(rows=ctx.rows, brief_text=ctx.brief_text)
    ):
        return _no_strategy(ctx)
    return ProductionStrategyAssessment(
        action="generate_rebuild_candidates",
...
def _rule_pass_prediction_rebuild(ctx: ProductionStrategyContext) -> ProductionStrategyAssessment:
    if not ctx.latest_quality or not ctx.revision_brief:
        return _no_strategy(ctx)
    prediction = predict_revision_pass(_loads_json(ctx.latest_quality.report), chapter_number=0)
    if not prediction.should_rebuild or prediction.confidence < 88:
        return _no_strategy(ctx)
    return ProductionStrategyAssessment(
        action="generate_rebuild_candidates",
...
def _rule_protected_context(ctx: ProductionStrategyContext) -> ProductionStrategyAssessment:
    if not (ctx.has_sample_adoption or ctx.has_continuity_context):
        return _no_strategy(ctx)
    return ProductionStrategyAssessment(
        action="",
