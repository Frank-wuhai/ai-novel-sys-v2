...
for i in $(seq 1 "$MAX_ROUNDS"); do
  ...
  venv/bin/python -m app.cli run-generation-worker --book-id "$BOOK_ID" --max-loops 6 --sleep-seconds 3 --max-tasks-per-loop 2 --recover-stale-before-run --task-timeout-seconds 3600 2>&1 | tee -a "$LOG" | tail -5
