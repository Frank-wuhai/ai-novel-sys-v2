#!/usr/bin/env bash
# Sprint 2 P1-3 stage-9 后：Ch16-20 长跑（5 章验证根因修复）
set -u
...
echo "=== Sprint 2 P1-3 stage-9 batch: Ch16-Ch20 ==="
echo "outdir=$OUTDIR"

for CH in 16 17 18 19 20; do
...
with session_scope() as s:
    ...
    if ch:
        ...
        print(f'STAT Ch$CH ch_status={ch.status} versions={len(vs)} v_id={v.id if v else None} score={q.score if q else None} passed={q.passed if q else None} rp_count={len(rp)} auto={auto_ok}')
" 2>&1 | tail -1
    echo ""
done
