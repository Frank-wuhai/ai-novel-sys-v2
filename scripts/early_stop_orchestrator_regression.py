...
5. Clean active story contracts block historical early-stop recovery.
6. ``early_stop_should_stop=False`` leaves the pre-existing revise route
   intact.
"""
...
def main() -> int:
    ...
    # 5. A clean active story contract must block early-stop recovery of an old
    #    high-scoring draft, then fall through to the contract route.
    situation = _base_situation(
        story_clean_revision_brief=True,
        early_stop_should_stop=True,
        early_stop_best_score=90,
        early_stop_best_version=27,
        early_stop_triggered_rules=("max_versions",),
    ...
    if route.action == "accept_early_stop":
        failures.append(
            "case5: clean revision contract must block accept_early_stop, "
            f"got action={route.action!r} intent={route.intent!r}"
        )

    # 6. early_stop_should_stop=False leaves the classic revise route.
    situation = _base_situation(
    ...
    if route.action != "revise_chapter":
        failures.append(f"case6: classic revise path broken, got {route.action!r}")

    # 7. Threshold exactly at 75 fires accept path.
    situation = _base_situation(
        early_stop_should_stop=True,
    ...
    route = decide_production_route(situation)
    if route.action != "accept_early_stop":
        failures.append(f"case7: score==75 boundary must accept, got {route.action!r}")

    # 8. Below threshold (74) with should_stop=True should escalate, not accept.
    situation = _base_situation(
        early_stop_should_stop=True,
    ...
    route = decide_production_route(situation)
    if route.action != "generate_rebuild_candidates":
        failures.append(f"case8: score=74 must escalate not accept, got {route.action!r}")
