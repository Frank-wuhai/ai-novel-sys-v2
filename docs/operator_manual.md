...
Queued generation tasks track attempts, model parameters, timeout settings, and retryable failures:
...
Each queued generation task can carry its own `task_timeout_seconds`. Queue health reports running age, timeout threshold, stale state, and whether the task can be recovered. If a task has no per-task timeout, recovery commands use their `--timeout-seconds` or `--task-timeout-seconds` fallback. Generation tasks also snapshot `llm_parameters` in JSON audit data, including provider mode, requested model, max tokens, and temperature.
...
After a feature iteration, inspect disposable generated artifacts before running more production work:

```bash
python scripts/system_trash.py
```

The dry run reports safe auto-move candidates such as `__pycache__`, isolated regression databases in `data/*regression.db`, smoke/debug databases, and optional old logs. It also reports tracked source files that contain legacy markers as `REVIEW` only; source files are never moved automatically.

To quarantine safe candidates into `data/trash/<timestamp>/` with a manifest:

```bash
python scripts/system_trash.py --apply --label after-iteration
```

To make cleanup automatic at the end of a regression run:

```bash
python scripts/run_regressions.py --skip-smoke --trash-after-pass
```

The cleanup only runs when blocking regressions pass. The shortcut script below runs the same safe end-of-iteration routine:

```bash
scripts/finish_iteration.sh
```

Old logs are skipped by default. Include them only when needed:

```bash
python scripts/system_trash.py --include-logs --log-retention-days 7
```

Every applied cleanup writes `manifest.json` under the trash folder so moved files can be inspected or restored manually.
